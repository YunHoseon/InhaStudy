...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
   ...
   HWND hWnd = CreateWindowW(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW,
      CW_USEDEFAULT, 0, 620, 640, nullptr, nullptr, hInstance, nullptr);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_LBUTTONDOWN:
	{
		setStart = (setStart == false) ? true : false;
		mousePt.x = LOWORD(lParam);
		mousePt.y = HIWORD(lParam);
		int* loc = map.CheckCellPt(mousePt);

		if (loc != NULL)
		{
			if (setStart == true)
			{
				map.board[loc[0]][loc[1]].cellState = CellState::START;
				startNend[0].y = loc[0];
				startNend[0].x = loc[1];
			}
			else if (setStart == false)
			{
				map.board[loc[0]][loc[1]].cellState = CellState::END;
				startNend[1].y = loc[0];
				startNend[1].x = loc[1];
				AStar(startNend);
			}
	...
	case WM_RBUTTONDOWN:
	{
		...
		if (loc != NULL)
			map.board[loc[0]][loc[1]].cellState = CellState::BLOCK;
		InvalidateRgn(hWnd, NULL, TRUE);
		break;
	}
