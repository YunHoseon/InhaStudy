	...
	while (1)
	{
		if (_kbhit())
		{
			...
			if (pressedKey == 224 || pressedKey == 0)
			{
				...
				{
				case 72:	// 위로 이동
					if (map[currentY - 1][currentX] != '1' && map[currentY - 1][currentX] != '|' && 
						map[currentY - 1][currentX] != '/' && map[currentY - 1][currentX] != '-')
					{
						currentY -= 1;
				...
				case 80:	// 아래로 이동
					...
					break;
				case 75:	// 왼쪽으로 이동
					if (map[currentY][currentX - 1] != '1' && map[currentY][currentX - 1] != '|' && 
...
void PrintMenu()
{
	printf("\t __   __  _______  ___   _  _______  __   __  _______  __    _ \n");
	printf("\t|  | |  ||       ||   | | ||       ||  |_|  ||       ||  |  | |\n");
	printf("\t|  |_|  ||   _   ||   |_| ||    ___||       ||   _   ||   |_| |\n");
	printf("\t|       ||  | |  ||      _||   |___ |       ||  | |  ||       |\n");
	printf("\t|       ||  |_|  ||     |_ |    ___||       ||  |_|  ||  _    |\n");
	printf("\t|   _   ||       ||    _  ||   |___ | ||_|| ||       || | |   |\n");
	printf("\t|__| |__||_______||___| |_||_______||_|   |_||_______||_|  |__|\n");
		...
		if (nInput2 == 1)
		{
			...
			if (myTurn == 0)
			{
				...
				if (enemy->GetmHp() == 0)
				{
					...
					Sleep(1000);
					getEx += 5;
					printf("\t%s는 %d 경험치를 얻었다!\n", Player.GetmyMonster(0)->GetmName(), 5);
					Sleep(2000);
					LevelUp();
					saveHp += 1;
					break;
...
void eSkillFunc(int rNum, char* RattataSName1, char* RattataSName2, int RattataSDamage, int RattataAtk)
{
	...
	if (strcmp(RattataSName1, "몸통박치기") == 0 && rNum == 0)
	{
...
void LevelUp()
{
	...
	if (upLevel != 0)
	{
		...
		Bulbasaur *mon = (Bulbasaur *)Player.GetmyMonster(0);
		mon->StatusUp();
